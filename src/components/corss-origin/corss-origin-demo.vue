...
<template>
	<div>
		...
		<div>
			...
			-----------
			<div class="gutter">
				<a href='./iframe.html'>跳转到iframe页面</a>
			</div>
		</div>
...
export default {
	name: 'corss-origin-demo',
	components: {},
	data() {
		return {
			tabs: [
				{ key: 'yanshi', label: '跨域演示' },
				{ key: 'module', label: '模块化' }
			],
			selectedTabIndex: 0,
			title: "这里是演示跨域的演示项目",
			res: ""
	...
	methods: {
		getUserByJsonp() {
			jsonp("http://127.0.0.1:3000/jsonp", {timeout: 10000, name: 'jQueryContent'}, (err, res) => {
				this.res = res;
			});
		},
		...
		onClickTab(idx) {
			...
			if (idx === 0) {
				...
				this.$router.push({path: '/module-demo'})
			}
		}
	}
};
